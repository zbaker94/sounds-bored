...
## Current Changes
- Internal code comments and documentation updated for clarity; no functional changes to app behavior.
...
- No user-facing changes in this update — all modifications are internal test suite improvements only.
...
- **Audio normalization & limiter**: Sounds now auto-normalize to -14 LUFS during playback; a per-pad brickwall limiter prevents clipping when multiple sounds play simultaneously; preview playback uses the same limiter pipeline, and quiet sounds can now be boosted beyond unity gain for more consistent volume across your library.
- **Sound analysis (loudness & genre/mood)**: Loudness and genre/mood analysis are now separate, independently-triggered actions; sounds can be analyzed on-demand (via an Analyze button when sounds are selected) or automatically when added to the library (Settings → Playback → Auto-analyze sounds); genre and mood labels appear under each sound and are searchable.
- **Analysis UX**: A real-time status indicator shows analysis progress with file counts and errors; running analyses can be cancelled from the status popover; a warning dialog appears before analyzing large (>50 MB) or already-analyzed sounds, with an option to skip the latter; each sound shows a dB adjustment pill reflecting its normalization gain.
- **Cover art**: Album artwork embedded in audio files (MP3, FLAC, OGG, M4A, WAV) is automatically extracted on library load, cached, and displayed throughout the app — blurred backgrounds on active pads, thumbnails in the sound library, and per-sound art on pad back faces and sound popovers.
- **Pad playback overlay & visual feedback**: Active pads crossfade from the pad name to a sound-name/layer/duration overlay when a sound starts; pads pulse with a ring animation while playing; volume percentage briefly appears when adjusting pad volume; cover art appears as a blurred background during playback.
- **Pad back face inline editor**: Right-clicking a pad flips it to reveal an inline editing surface for name, color, layers, and fade settings, replacing the separate config drawer; per-layer controls include play/stop, volume, and skip forward/back in sequential arrangements; new pads immediately open in edit mode.
- **Move & copy pads between scenes**: Pads can be moved or copied to another scene directly from the back face, with a scene selector appearing when multiple scenes exist; copied pads retain all layers as independent duplicates.
- **Fade controls redesign**: Fades simplified to a volume + fade-target model; pressing **F** sets the target before firing, pressing again executes; **Stop Fade** and **Reverse** buttons appear mid-fade; fading in a stopped pad ramps from its configured low-volume boundary; existing projects auto-migrate to the new format (v1.3.0).
- **Keyboard shortcuts**: Added **Shift+Left** / **Shift+Right** to page through the pad grid (each scene remembers its page); **Mod+Shift+N** navigates to the new pad's page and opens it in edit mode; **F** / **X** trigger fade and multi-fade from a pad's popover or back face; scene navigation moved to **Alt+Left** / **Alt+Right** to prevent conflicts with inputs and sliders.
- **Download manager**: Persistent download history survives restarts; tags and sets can be assigned before a download starts; the toolbar shows an active-download badge; improved error reporting reflects actual yt-dlp failure messages; download dialog stays open with input preserved after a failed attempt.
- **Security hardening**: File system access is now scoped to explicitly-chosen folders only (no broad home/Music/Documents access at startup); path traversal blocked in downloads and project exports; yt-dlp runs in isolated mode; UNC/device-namespace paths and BIDI/null-byte characters rejected; download job IDs and URLs strictly validated.
- **Performance**: Audio tick loop skips all work for idle pads and skips Zustand updates on frames with no state changes; pulse ring animation runs entirely in CSS; pad render subscriptions unmount when not in edit mode; layer volume sliders throttled to ~10 Hz during fades; tag/set sound selections cached across render cycles; pad lookups use O(1) map instead of linear search.
- **Auto-save & data integrity**: All project and library saves are now atomic (crash-safe); auto-save correctly restarts on config changes and retries failed saves with a rate-limited error toast; corrupted data files (library, history, download history) are automatically backed up and reset with a warning notification.
- **Audio engine reliability**: Per-layer playback errors are isolated and surfaced as toast notifications rather than crashing other layers; stale playing/fading indicators are reliably cleared on stop, retrigger, or project close; retrigger and stop race conditions fixed; sequential sound chains now correctly advance to the next sound.
- **Tag & set pickers + bug fixes**: Reusable `LibraryItemPicker` with inline create is now shared across all tag/set selection UI (Add Tags dialog, Download dialog, pad config); partial tag selections show a minus-sign indicator; fixed layer config showing stale data when switching layers, scene switching flicker, last-scene crash, and missing-sounds double-notification after Save As.
