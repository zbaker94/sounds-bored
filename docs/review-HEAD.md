...
- **Fix applied**: Created `PadLabeledSlider` with `{ min, max, step, formatValue: (v: number) => string }` props. Both `PadPercentSlider` and `PadDurationSlider` deleted; `PadFadeControls` updated to use `PadLabeledSlider` at both call sites. The dark-overlay slider pattern (label below slider, `text-[9px] text-white/70`, white-tinted track classes, `onPointerUp` semantics) was extracted into a separate `PadOverlaySlider` component at `src/components/composite/SceneView/PadOverlaySlider.tsx`; `PadButtonFadeOverlay` migrated to use `PadOverlaySlider` for all 3 sliders (volume, target, fade duration). `PadLabeledSlider` gained an optional `onThumbPointerDown?: (index: number) => void` prop, and the inline current-volume slider in `PadFadeControls` was migrated to `PadLabeledSlider` via that prop — eliminating the last inline duplication of the labeled slider pattern.
...
| REUSE5 | `PadPercentSlider` and `PadDurationSlider` collapsed into `PadLabeledSlider`; both old files deleted; `PadFadeControls` updated; `PadOverlaySlider` extracted for dark-overlay pattern (label-below, white text); `PadButtonFadeOverlay` migrated to `PadOverlaySlider`; `PadLabeledSlider` extended with `onThumbPointerDown`; `PadFadeControls` current-volume slider migrated to `PadLabeledSlider` |
