...
export function isSoundLoudnessAnalyzed(sound: Sound): boolean {
  return sound.loudnessLufs !== undefined;
}

function countAnalyzed(sounds: Sound[]): number {
  return sounds.filter(isSoundLoudnessAnalyzed).length;
}
...
export function AnalysisWarningDialog({
  ...
  const analyzedCount = countAnalyzed(sounds);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onCancel(); }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Analyze Loudness — {sounds.length} sound{sounds.length !== 1 ? "s" : ""}?</DialogTitle>
          {hasLargeFiles && (
            <DialogDescription>
...
export function shouldWarnBeforeLoudnessAnalysis(sounds: Sound[]): boolean {
  const hasAnalyzed = sounds.some(isSoundLoudnessAnalyzed);
  const totalBytes = sounds.reduce((sum, s) => sum + (s.fileSizeBytes ?? 0), 0);
  const hasLargeFiles =
    sounds.some((s) => (s.fileSizeBytes ?? 0) >= ANALYSIS_LARGE_FILE_BYTES) ||
    totalBytes >= ANALYSIS_LARGE_TOTAL_BYTES;
